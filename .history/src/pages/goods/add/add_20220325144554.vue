...
export default {
  ...
  methods: {
    ...
    bindPickerType(e) {
      ...
      if(this.typeIndex == 6 || this.typeIndex == 8 || this.typeIndex == 9) {
    ...
    submit() {
      ...
      if (String(data.weight).replace(/\s/g, "") <= 0 || !this.volumeShow) {
        return uni.showToast({
          title: "缺少重量",
          icon: "none"
        });
      }
      if (volumeOnes < 2 || !volumeOnes&&this.volumeShow) {
        return uni.showToast({
          title: "请输入真实长度",
          icon: "none"
        });
      }
      if (volumeTwos < 2 || !volumeTwos || this.volumeShow) {
        return uni.showToast({
          title: "请输入真实宽度",
          icon: "none"
        });
      }
      if (volumeThrees < 2 || !volumeThrees || this.volumeShow) {
        return uni.showToast({
          title: "请输入真实高度",
          icon: "none"
        });
