<template>
  ...
  >
    ...
    <view style="height:100%;">
      <block v-if="detail">
        <swiper vertical :current="currentIndex" style="height:calc(100% - 112rpx);">
          <swiper-item>
              ...
              <view class="container">
                <view class="avatar-info flx fx-middle fx-justify">
                  ...
                  >
                    <image
                      ...
                      :class="{'charity':detail.seller && detail.seller.wisher}"
                    />
                    <image
                      v-if="detail.seller && detail.seller.wisher"
                      mode="widthFix"
                  ...
                  >
                    ...
                    />
                    <text style="color:#24775F;font-size:14px;">举报</text>
                  </view>-->
                </view>
                <navigator
                  class="flx fx-middle"
                ...
                <view class="detail">
                  <view class="title flx">
                    <text v-if="detail.seller && detail.seller.wisher" class="label new">公益</text>
                  ...
                  <view class="label">
                    <view class="label-li">
                      <view class="label-key">收货地点：</view>
                      <view>{{detail.place[0]}}{{detail.place[1]}}{{detail.place[2]}}</view>
                    </view>
                  </view>
                  <view class="label">
                    <view class="label-li" @click="copy(detail.seller_address.telephone)">
                      <view class="label-key">联系方式：</view>
                      <view>{{detail.seller_address.telephone}}</view>
                      <image
                        mode="widthFix"
                        :src="imgHOST+'/icon/copy.png'"
                        style="margin-left:20rpx;width:30rpx;height:30rpx;"
                      />
                    </view>
                  </view>
                  <view class="imgs-content">
                    <block v-for="(item,index) in detail.pic" :key="index">
                      <image mode="widthFix" :src="item" @click="toView(index)" :data-index="index" />
                    </block>
                    <!-- <view class="info">
                          <text>10人想要</text>
          ...
          <block v-if="false">
            <swiper-item>
              <view id="makereadmsg" class="container" style="height:100%;">
                <view class="message" style="height:100%;">
                  <view class="title">
                    <block v-if="detail.only_pickup && isOwner">
                      <view
                        class="tab-li"
                        :class="[currentMessageIndex == 0 ? 'on' : '']"
                      ...
                      <view
                        ...
                        class="tab-li"
                        :class="[currentMessageIndex == 1 ? 'on' : '']"
                              ...
                              <view class="list flx">
                                ...
                                <view class="txt">
                                  ...
                                  <view class="message-1">
                                    ...
                                    <view class="copy-btn" @click="copy(item.contact)">复制手机</view>
                                  </view>
                                </view>
                              </view>
                            </view>
          ...
          <block v-if="isOwner">
            <block v-if="detail.status==1">
              ...
              <view class="flx fx-middle">
                <navigator hover-class="none" :url="'/pages/wish/add?id='+id" class="icon-txt">
                ...
                <view class="icon-txt" @click="previewWechat(imgHOST+'/断舍哩进群群二维码.jpg')">
                ...
                <!-- <view class="icon-txt" @click="toggleFav">
                  ...
                  <view>{{detail.isFav ? '已收藏' : ''}}</view>
                </view>-->
              </view>
              <view class="flx">
                <button open-type="share" class="btn">分享好物</button>
              </view>
            </block>
          </block>
          <block v-else>
            <view class="flx fx-middle">
              ...
              <button open-type="share" class="icon-txt">
            ...
            <block v-if="detail.status == 1">
              <view class="btn" @click="()=>isShowDonateModal=true">去捐赠</view>
            </block>
            ...
            <block v-else>
              <view class="btn disable">已失效</view>
            </block>
          </block>
        </view>
        ...
        <action-sheet share :on="isShowShare"></action-sheet>
      </block>
      <block v-if="isNull">
        <view
    ...
    <!-- donate modal -->
    <view v-if="isShowDonateModal" class="audit-modal">
      <view class="audit-container">
        <view class="label">物品描述</view>
        <textarea v-model="donateData.description" placeholder="请填写赠送物品的简单情况~"></textarea>
        <view class="label">联系方式</view>
        <textarea
          v-model="donateData.phone"
          placeholder="请填写您的微信号或手机号等，方便联系到您哩~"
          style="height:120rpx;"
        ></textarea>
        <view class="btns">
          <view class="btn reject" @click="()=>isShowDonateModal=false">取消</view>
          <view class="btn accept" @click="toDonate">提交</view>
        </view>
      </view>
    </view>
    ...
    <view v-if="isShowWechatGroupModal" class="wechat-group-modal">
      ...
      />-->
      <image mode="widthFix" :src="imgHOST+'/断舍哩进群群二维码.jpg'" @longtap.stop="saveGroupQR" />
    </view>
...
export default {
  ...
  methods: {
    ...
    toDonate() {
      let url = "/wish/donate",
        data = this.donateData;
      data.order_id = this.id;
      if (!data.description || !data.phone) {
    ...
    previewWechat() {
      uni.previewImage({
        current: imgHOST + "/断舍哩进群群二维码.jpg", // 当前显示图片的http链接
        urls: [imgHOST + "/断舍哩进群群二维码.jpg"] // 需要预览的图片http链接列表
      });
    },
    copy(phone) {
      uni.setClipboardData({
  ...
  onShareAppMessage(res) {
    ...
    return {
      ...
      imageUrl: this.detail.pic[0] || shareContent.wishImg,
      path: "/pages/wish/detail?id=" + this.id
    };
  },
  onShareTimeline(res) {
    return {
