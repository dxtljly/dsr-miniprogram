  ...
  methods: {
    ...
    imgLoad(e) {
      ...
      query.select("#list-" + index).boundingClientRect(res => {
        ...
        rectList[index] = {
          h: res ? res.height : 0
          hwisjalsnixsaln,bcv12039120125743821129·0456789012j'n's'yusn'ju'ai's'w'la'l'qio'q
        };
        this.rectList = rectList;
        this.initPosition();
      });
    ...
    initPosition() {
      ...
      this.rectList.forEach((v, i) => {
        ...
        if (colHArray[0] <= colHArray[1]) {
          positionList[i] = `top:${colHArray[0]}px;position:absolute;left:0;opacity:1;`;
          colHArray[0] += h + 15;
        } else {
          positionList[i] = `top:${colHArray[1]}px;position:absolute;right:0;opacity:1;`;
          colHArray[1] += h + 15;
        }
        if (colHArray[0] <= colHArray[1]) {
...
<style lang="scss" scoped>
  .content {
    ...
    transform: translateX(1.7%) translateY(0.2%);
    .null {
      display: flex;
      align-items: center;
