  ...
  <view style="height:100%;background:#fff;">
    <view v-for="(item5, index5) in config.school" :key="index5">
      <block v-if="item5.page">
        ...
        <image
          mode="widthFix"
          :src="imgHOST + '/applyID/header.png'"
        ...
        <view
          class="id-img-content"
          v-for="(item1, index1) in config.school"
          :key="index1"
        >
        ...
        <view
          class="tip"
          v-for="(item2, index2) in config.school"
          :key="index2"
        >
          <image
            mode="widthFix"
      ...
      xhr.post(url, data, (res) => {
        ...
        if (res.statusCode == 200) {
          uni.requestSubscribeMessage({
            tmplIds: ['QN_njQ35HmgPO2yWmBzlq_OkbElUg50Gy-OSAmnyReQ'],
            success: (res) => {},
            fail: (res) => {},
            complete: (res) => {
              uni.showModal({
