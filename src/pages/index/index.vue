<template>
  ...
  >
    ...
    <view v-if="isShowSpread && config && config.spread" class="spread" @touchstart="touchSpread">
        <view class="content">
          <view
            ...
            @longtap="saveImg(imgHOST + '/双十一开屏(有).jpg')"
          >
            <image mode="widthFix" :src="imgHOST+'/双十一开屏(有).jpg'" />
            <!-- <view
              class="close"
              @click="closeSpread"
              :style="'top:'+(statusBarHeight+navigationHeight+20)+'px;'"
            >关闭</view> -->
            <view v-if="!isTouchSpread && config.downloadImg" class="save-tips">长按保存</view>
          </view>
        </view>
      <view
        class="content"
        ...
        @touchstart="()=>isTouchSpread=true"
        @longtap="saveImg(config.spread.downloadImg)"
      >
    ...
    <!-- <view v-if="isShowGifModal" class="gif-modal">
      <view class="bg" @click="closeGifModal"></view>
      <image mode="widthFix" :src="imgHOST+'/送弹窗.jpg'" class="z-index-1 card" @click="copyWX" />
    </view>-->
    <!-- <view v-if="isShowGifModal" class="gif-modal">
      <view class="bg" @click="closeGifModal"></view>
      <navigator
        hover-class="none"
        url="/pages/webviews/webviews?url=https://image.grecycle.com.cn/src/sli/html/日置名媛.html"
        style="z-index:99999;"
      >
...
export default {
  ...
  methods: {
    ...
    getConfig() {
      let url =
          "https://image.grecycle.com.cn/src/sli/config/wx-index-config.json",
        data = {};
